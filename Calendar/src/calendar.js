...
(function (root, factory) {
    ...
    var defaults = {
        ...
        OFFDAY_HTML = '<i class="offday">休</i>',        //休息日
        WORKDAY_HTML = '<i class="workday">班</i>',       //工作日
  ...
  　function removeByVal(val, arr) {
    ...
    Calendar.prototype = {
        ...
        getDayItem: function (y, m, d, f) {
            ...
            dateType = this.getDayType(idt);  //类型  休假还是 工作日  暂时两个分类  1---休息  2---工作日
            ...
            if (markData) {
                ...
                if(dateType) {
                  switch(dateType) {
                    case '1': $item.html(d + OFFDAY_HTML);break;
                    case '2': $item.html(d + WORKDAY_HTML);break;
                    default: $item.html(d + MARK_DAY_HTML);break;
                  }
                }else {
        ...
        event: function () {
            ...
            var ctrlKeyDown = false, multiArray = this.multiArray;
            ...
            _this.$element.on('click', '[' + ITEM_DAY + ']', function (e) {
                ...
                // support multi-selected
                if(ctrlKeyDown) {

                  if($(this)[0].tagName.toUpperCase() === 'LI') {

                    $(this).toggleClass('selected'); // toggle selected class
                    var day = $(this).attr('data-calendar-day'),   // get the day info
                    isSelected = $(this).hasClass('selected');  //is selected status after click

                    isSelected ? multiArray.push(day) : removeByVal(day, multiArray);
                    _this.options.onMultiSelected.call(this, multiArray)
                  }
                }else {
                  multiArray.splice(0, multiArray.length);
                  multiArray.push($(this).attr('data-calendar-day'))
                  var day = _this.selectedDay(d, type);
                  _this.options.onSelected.call(this, 'date', day, $(this).data(MARK_DATA));
                  _this.$trigger && _this.hide('date', day, $(this).data(MARK_DATA));
                }
